...
public class BigGrayMushroomFeature extends AbstractHugeMushroomFeature {
	 ...
	 */
	private void placeBlock(LevelAccessor worldIn, Random rand, BlockPos pos, int heightIn, MutableBlockPos mutableIn, HugeMushroomFeatureConfiguration config, int height, int x, int z, int radius) {
		if (this.canBeReplacedByLeaves(worldIn, mutableIn)) {
			this.setBlock(worldIn, mutableIn, config.capProvider.getState(rand, pos)
				.setValue(HugeMushroomBlock.DOWN, Boolean.valueOf(height < heightIn -1))
				.setValue(HugeMushroomBlock.WEST, Boolean.valueOf(x <= 0 || Math.abs(z) == 2))
				.setValue(HugeMushroomBlock.EAST, Boolean.valueOf(x >= 0 || Math.abs(z) == 2))
				.setValue(HugeMushroomBlock.NORTH, Boolean.valueOf(z <= 0 || Math.abs(x) == 2))
				.setValue(HugeMushroomBlock.SOUTH, Boolean.valueOf(z >= 0 || Math.abs(x) == 2)));
		}
	}
