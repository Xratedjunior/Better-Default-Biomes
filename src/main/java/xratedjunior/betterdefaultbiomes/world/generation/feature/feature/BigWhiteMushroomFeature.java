...
public class BigWhiteMushroomFeature extends AbstractHugeMushroomFeature {
	...
	private void placeBlock(LevelAccessor worldIn, Random rand, BlockPos pos, int heightIn, MutableBlockPos mutableIn, HugeMushroomFeatureConfiguration config, int height, int x, int z, int insideRadius) {
		this.setBlock(worldIn, mutableIn, config.capProvider.getState(rand, pos)
			...
			.setValue(HugeMushroomBlock.WEST, Boolean.valueOf(x < -insideRadius))
			.setValue(HugeMushroomBlock.EAST, Boolean.valueOf(x > insideRadius))
			.setValue(HugeMushroomBlock.NORTH, Boolean.valueOf(z < -insideRadius))
			.setValue(HugeMushroomBlock.SOUTH, Boolean.valueOf(z > insideRadius)));
	}
}
